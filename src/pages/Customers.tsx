...
const Customers: React.FC = () => {
    ...
    const handleAddressChange = (val: string) => {
        setAddress(val);
        if (sameAsBilling) setShippingAddress(val);
    };
    ...
    const handleSave = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!name.trim() || !email.trim() || !phone.trim() || !address.trim() || !placeOfSupply || !pinCode.trim()) {
                    ...
                    <form onSubmit={handleSave}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input type="text" className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none" value={contactPerson} onChange={(e) => setContactPerson(e.target.value)} placeholder="John Doe (Optional)" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Email *</label>
                                <input type="email" required className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="contact@acme.com" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-1">Phone *</label>
                                <input type="tel" required className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91..." />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4 border-t border-gray-100 pt-4">
                            <div>
                                ...
                                <textarea rows={3} required className="w-full border border-gray-300 rounded-lg p-2 text-sm resize-none focus:ring-indigo-500 focus:border-indigo-500 outline-none" value={address} onChange={(e) => handleAddressChange(e.target.value)} placeholder="Street, Building" />
                            </div>
                            <div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                            <div>
                                ...
                                <select required className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none bg-white" value={placeOfSupply} onChange={(e) => setPlaceOfSupply(e.target.value)}>
                            ...
                            <div>
                                ...
                                <input type="text" required className="w-full border border-gray-300 rounded-lg p-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 outline-none" value={pinCode} onChange={(e) => setPinCode(e.target.value)} placeholder="110001" maxLength={6} />
                            </div>
                            <div>
                                    ...
                                    {customers.map((c) => (
                                        <tr key={c.id} className={`hover:bg-gray-50 ${editingId === c.id ? 'bg-indigo-50' : ''}`}>
                                            <td className="px-6 py-3 font-medium text-gray-900">
                                                ...
                                                {c.placeOfSupply && <div className="text-[10px] bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded inline-block mt-1 mr-1">{c.placeOfSupply}</div>}
                                                {c.gstin && <div className="text-[10px] bg-blue-50 text-blue-700 px-1.5 py-0.5 rounded inline-block mt-1 border border-blue-100">GST: {c.gstin}</div>}
                                            </td>
