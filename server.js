...
app.post('/api/notify', async (req, res) => {
    ...
    try {
        ...
        const info = await transporter.sendMail({
            ...
            to: to, subject: subject, html: body, attachments: attachments
        });
...
app.post('/api/whatsapp/send', async (req, res) => {
    ...
    if (!apiKey) return res.json({ success: true, message: "Stub: API Key missing" });
    ...
    if (triggerType === 'PAID') {
        payload = {
            from: fromNumber, to: recipient, type: "template",
            template: {
                name: "payment_rcv_inv", language: { code: "en", policy: "deterministic" },
                components: [{ type: "header", parameters: [{ type: "document", document: { link: link, filename: "Invoice_from_Wappie.pdf" } }] }, { type: "body", parameters: [{ type: "text", text: buyerName || "Customer" }, { type: "text", text: amount.toString() }, { type: "text", text: invoiceNumber }] }]
            }
        };
    } else {
        payload = {
            from: fromNumber, to: recipient, type: "template",
            template: {
                name: "inv_quote_status", language: { code: "en", "policy": "deterministic" },
                components: [{ type: "body", parameters: [{ type: "text", text: buyerName || "Customer" }, { type: "text", text: invoiceNumber }, { type: "text", text: amount.toString() }, { type: "text", text: link }] }]
            }
        };
    }
    try {
...
app.post('/api/payment/razorpay/create-order', async (req, res) => {
    ...
    try {
        const order = await razorpay.orders.create({ amount: Math.round(amount * 100), currency: currency || 'INR', receipt: receipt });
...
app.post('/api/payment/initiate', (req, res) => {
    ...
    paramsMap.append('merchant_id', merchantId); paramsMap.append('order_id', order_id); paramsMap.append('currency', currency); paramsMap.append('amount', amount); paramsMap.append('redirect_url', redirectUrl); paramsMap.append('cancel_url', cancelUrl); paramsMap.append('language', 'EN'); paramsMap.append('billing_name', billing_name); paramsMap.append('billing_address', billing_address); paramsMap.append('billing_email', email); paramsMap.append('billing_tel', billing_tel);
    const encRequest = ccav.encrypt(paramsMap.toString(), workingKey);
    const form = `<html><head><title>Redirecting...</title></head><body><center><h2>Please wait...</h2><form id="nonseamless" method="post" name="redirect" action="https://secure.ccavenue.com/transaction/transaction.do?command=initiateTransaction"><input type="hidden" name="encRequest" value="${encRequest}"><input type="hidden" name="access_code" value="${accessCode}"></form><script>document.redirect.submit();</script></center></body></html>`;
    res.send(form);
});
...
app.post('/api/payment/ccavResponseHandler', async (req, res) => {
    ...
    const workingKey = process.env.CCAV_WORKING_KEY;
    let decrypted = '';
    try { decrypted = ccav.decrypt(encResp, workingKey); } catch(e) { return res.send("<script>window.close();</script>"); }
    ...
    if (orderStatus && orderStatus.toLowerCase() === 'success') {
        ...
        res.send(`<script>if(window.opener){window.opener.postMessage('PAYMENT_SUCCESS', '*');} window.close();</script>`);
    } else {
        await pool.query('UPDATE Invoices SET Status = ? WHERE ID = ?', ['FAILED', orderId]);
        res.send(`<script>if(window.opener){window.opener.postMessage('PAYMENT_CANCEL', '*');} window.close();</script>`);
    }
});
